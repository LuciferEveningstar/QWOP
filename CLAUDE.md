...
16. **Headless-Training + Parallelisierung (verifiziert Juli 2026).** qwop-gym startet Chrome per Default sichtbar und bietet **keinen** Headless-Kwarg (`WSServer._launch_browser` hardcodet die ChromeOptions). Headless geht trotzdem — via Monkey-Patch. Nutzen: umgeht die Vordergrund-Drosselung (Stolperstein 12) → echte Multi-Env-Parallelität möglich.
    ...
    - **Produktion:** Der Headless-Patch lebt in `src/qwop_rl/envs/_headless.py` (`apply_headless_patch()`, idempotent). `scripts/train.py` setzt bei `n_envs>1` automatisch `QWOP_HEADLESS=1`.
    - **Enkel-Falle — der Worker-Patch reicht NICHT (verifiziert):** qwop-gym startet den Browser nicht im SB3-Worker, sondern in einem **Enkelprozess** (`qwop_env.py:177`: `multiprocessing.Process(target=server.start)`, Default-Kontext `spawn` auf macOS). Der Enkel re-importiert `WSServer` frisch → ein nur im Worker gesetzter Patch ist weg → **sichtbare Fenster** (4 statt 0). **Fix: `.pth`-Startup-Hook.** `scripts/install_headless_hook.py` schreibt `qwop_rl_headless.pth` in site-packages; deren Import-Zeile lädt `qwop_rl._headless_bootstrap` beim Start JEDES Interpreters (inkl. Enkel), das bei `QWOP_HEADLESS=1` den Patch appliziert. **Einmal nach `pip install -e .` ausführen** (maschinenlokal, überlebt keinen venv-Rebuild — siehe SETUP.md). Ohne den Hook starten die 4 Worker sichtbar und die Drosselung ist zurück.
    - **Kein `fork`-Workaround:** ein `fork` für den Enkel würde den Worker-Patch zwar erben, scheitert aber an fork-after-spawn mit geladenem PyTorch/objc (die `objc[...] fork()`-Warnung). Alles bleibt `spawn`.
    - **`n_envs=1` bleibt sichtbar** (Debugging, `QWOP_HEADLESS` unset → `.pth`-Hook = No-op). Ab `n_envs>1` → SubprocVecEnv + Auto-Headless.
